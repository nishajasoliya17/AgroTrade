...
if($_SERVER['REQUEST_METHOD'] == "POST" || $_SERVER['REQUEST_METHOD'] == "GET")
{
  ...
  $pay_type= $_REQUEST['pay_type'];
    ...
    {
        $total =  $row['total'];
        $tcost = ($row['total'] * 10)/100;
	  	  $rctpt = "RCPT".rand(000,999);
       $purchaseMaster = mysqli_query($obj->con,"insert into  salemaster_crop set
       cust_id ='".$user_id."',
       receipt_no ='".$rctpt."',
       ...
       $salemaster_cropID= mysqli_insert_id($obj->con);
       $data = array();

       $data['img'] = $obj -> generateQrCode($salemaster_cropID);
       
       $obj->updateData($table,$data,array("id" => $salemaster_cropID));

       $mqr = mysqli_query($obj->con,"select * from dummy where user_id='".$user_id."'");

        while($mrow = mysqli_fetch_array($mqr))
        {
          $purdetail = mysqli_query($obj->con,"insert into saledetail_crop set
           scp_id = '".$salemaster_cropID."',           
           pro_id = '".$mrow['pro_id']."',
           qty = '".$mrow['qty']."',
    ...
    if($pay_type == 'online')
    {
      if($dummy)
      {
        $response = array("data" => 200 , "url" => 'http://service.techfusiontechnologies.com/agrofarm/console/payment/?amount='.$total.'&order_id='.$salemaster_cropID.'');
      }
      else
      {
        $response = array("data" => 100);
